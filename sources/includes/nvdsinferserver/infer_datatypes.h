...
public:
    ...
    virtual NvDsInferStatus getDouble(
        const std::string& name, double& v) const = 0;
    virtual NvDsInferStatus getInt(
        const std::string& name, int64_t& v) const = 0;
    virtual NvDsInferStatus getUInt(
        const std::string& name, uint64_t& v) const = 0;
    virtual NvDsInferStatus getString(
        const std::string& name, std::string& v) const = 0;
    virtual NvDsInferStatus getBool(const std::string& name, bool& v) const = 0;
};

#define OPTION_SEQUENCE_ID "sequence_id"  // uint64

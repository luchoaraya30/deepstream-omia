...
std::vector<bool> parsed_lcs(10, true);
std::vector<bool> parsed_rois(10, true);
std::vector<string> lc_names;
std::vector<string> roi_names;

map<int, vector<string>> mapped_lcs;
map<int, vector<string>> mapped_rois;

static const gchar* to_str (gchar* cstr);
void parseAnalyticBySource(int source, char analytic);
...
void parseAnalyticBySource(int source, char analytic, std::string input) {
	char *arr = &input[0];
	std::vector<string> names = splitString(arr);
	
	if (analytic == 'l'){
		mapped_lcs[source] = names;
		parsed_lcs[source] = false;
	} else if (analytic == 'r') {
		mapped_rois[source] = names;
		parsed_rois[source] = false;
	}
}

int getAnalyticMeta(char analytic, std::string input , gint source_id, const gchar *name) {
	if (analytic == 'l' && parsed_lcs[source_id])
		parseAnalyticBySource(source_id, 'l', input);
	else if (analytic == 'r' && parsed_rois[source_id])
		parseAnalyticBySource(source_id, 'r', input);

	if (analytic == 'l') {
        	for (int i = 0; i < (int) mapped_lcs[source_id].size(); i++) {
                	if (name == mapped_lcs[source_id][i])
                        	return i;
        	}
	} 
	else if (analytic == 'r') {
                for (unsigned int i = 0; i < mapped_rois[source_id].size(); i++) {
                        if (name == mapped_rois[source_id][i]) 
                                return i;
                }
        }
	return -1;
...
generate_analytics_module_object (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  //---------------------- CUSTOM CODE --------------------//
  if (meta->lc_name != NULL)
	  meta->lc_id = getAnalyticMeta('l', dsObj->lc, meta->source_id, meta->lc_name);

  if (meta->roi_name != NULL)
	  meta->roi_id = getAnalyticMeta('r', dsObj->roi, meta->source_id, meta->roi_name);
  
  //cout << meta->lc_id << endl;
  // analytics object
  ...
  json_object_set_double_member (jobject, "width", meta->width);
  ...
  // Analytics meta
  jobject = json_object_new();
  json_object_set_int_member (jobject, "lc", meta->lc_id);
  json_object_set_int_member (jobject, "roi", meta->roi_id);
  json_object_set_object_member (analyticsObj, "analytics", jobject);
